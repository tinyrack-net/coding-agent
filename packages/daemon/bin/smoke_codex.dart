/// Manual end-to-end smoke over the real WebSocket API + real codex CLI.
///
/// Starts the daemon on an ephemeral port, connects like the app would,
/// creates a codex agent (mode: normal) in a temp dir, prompts it to write
/// pong.txt, auto-allows approvals over permission.respond, and verifies the
/// file. Exits 0 on success.
///
/// Note: requires a logged-in codex CLI with available quota; quota errors
...
Future<void> main() async {
  final tempDir = Directory.systemTemp.createTempSync('smoke-codex-');
  final dataDir = Directory.systemTemp.createTempSync('smoke-codex-data-');
  const port = 6898;
    ...
    if (f['type'] == MessageTypes.agentStreamEvent) {
      ...
      final summary = switch (item) {
        ...
        TurnItem(:final phase, :final errorMessage) =>
          'turn ${phase.name}${errorMessage == null ? '' : ' ($errorMessage)'}',
        _ => item.kind,
      };
  ...
  await request(MessageTypes.clientHelloRequest,
      const ClientHello(clientName: 'smoke-codex', clientVersion: '0').toJson());
  ...
  stdout.writeln('[smoke] created codex agent ${agent.agentId}');
