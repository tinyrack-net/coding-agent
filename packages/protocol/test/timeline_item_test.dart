...
void main() {
  group('UserMessageItem', () {
    test('round-trips', () {
      const item = UserMessageItem(id: 'i1', text: 'hello');
    ...
    test('fromJson defaults text to empty string', () {
      final decoded = TimelineItem.fromJson(
        const {'id': 'i1', 'kind': 'user_message'},
      );
  ...
  group('AssistantMessageItem', () {
    test('round-trips complete and incomplete', () {
      const complete =
          AssistantMessageItem(id: 'i1', text: 'done', complete: true);
      const streaming =
          AssistantMessageItem(id: 'i2', text: 'partial', complete: false);
    ...
    test('fromJson defaults complete to true when missing', () {
      final decoded = TimelineItem.fromJson(
        const {'id': 'i1', 'kind': 'assistant_message'},
      );
  ...
  group('ReasoningItem', () {
    test('round-trips', () {
      const item = ReasoningItem(id: 'i1', text: 'thinking...', complete: false);
      final decoded = TimelineItem.fromJson(roundTrip(item.toJson()));
    ...
    test('fromJson throws on unknown status', () {
      expect(
        () => TimelineItem.fromJson(
          const {'id': 'i1', 'kind': 'tool_call', 'status': 'bogus'},
        ),
        throwsArgumentError,
      );
    });
