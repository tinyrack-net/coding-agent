...
void main() {
  test('probeDaemon returns the ServerHello on a successful handshake',
      () async {
  ...
  test('probeDaemon returns null when nothing is listening', () async {
    ...
    final hello = await probeDaemon('127.0.0.1', 1,
        timeout: const Duration(milliseconds: 300));
    expect(hello, isNull);
  });

  test('probeDaemon returns null when the server never answers hello',
      () async {
  ...
  test('sendLifecycleRequest returns true on a non-error response', () async {
    ...
    final ok = await sendLifecycleRequest(
        '127.0.0.1', server.port, MessageTypes.daemonShutdownRequest);

    expect(ok, isTrue);
  });

  test('sendLifecycleRequest returns false for an unhandled request type',
      () async {
    final server = FakeDaemonServer();
    await server.start();
    addTearDown(server.stop);
    ...
    final ok = await sendLifecycleRequest(
        '127.0.0.1', server.port, 'some.unknown.request');

    expect(ok, isFalse);
  });

  test('sendLifecycleRequest returns false when nothing is listening',
      () async {
    final ok = await sendLifecycleRequest(
      '127.0.0.1',
      1,
